...
void MyOta::init (Config *configptr) {
    this->pC = configptr;
}
    
int MyOta::check_and_update() { 
   if (WiFi.status() != WL_CONNECTED) {
        SERIAL_PRINTLN("MyOta: No wifi connection !");        
        return -1;   // all positive status codes are taken by ESPhttpUpdate.update :)
    }
    SERIAL_PRINT("MyOta: Checking for new firmware ...");
