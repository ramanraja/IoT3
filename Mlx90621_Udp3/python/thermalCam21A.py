...
# read MLX90621 thermal sensor & detect objects by gray scale thresholding
# use it with the Arduino program  MLX90621_Udp2.ino
# gray scale operation, Otsu threshold, contour finding, and
# running average of people count to suppress noise - Works well **
...
# false-color palette: gray scale
...
while True:
    try:
        ...
        blurred = cv2.erode(blurred , None, iterations=3)
        blurred = cv2.dilate(blurred , None, iterations=3)
        
        # For contour finding the background should be black 
        ...
        # For contour finding the background should be black            
        #retval,the_mask = cv2.threshold (blurred, 100,255,  cv2.THRESH_BINARY_INV) 
        #retval,the_mask = cv2.threshold (blurred, 100,255,  cv2.THRESH_BINARY)   
        ...
        if (ccount > 0):
            ...
            cv2.drawContours(gray_frame, cntrs, -1, (0, 255, 0), 2)
            #rect = np.int32(cv2.BoxPoints(cv2.minAreaRect(cntrs)))
            #cv2.drawContours(gray_frame, [rect], -1, (0, 255, 0), 2)
            i = 0
            for c in cntrs:
