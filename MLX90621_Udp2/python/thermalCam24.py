...
# automatic calibration of min/max temperatures using running averages
...
CALIBRATION_INTERVAL = 100  # autocalibration frequency in terms of frames
NOISE_THRESHOLD = 3         # to avoid noise when the room is empty
NOISE_PEDESTAL = 2          # add this to the max_temp to keep a high margin
...
CALIBRATION_INTERVAL = cv_settings['autocal_interval']
...
N1 = 15
c_buffer = np.zeros(N1)
c_index = 0 

# running average of min/max temperatures
N2 = 20
mint_buffer = np.zeros(N2)
maxt_buffer = np.zeros(N2)
t_index = 0 
...
while True:
    try:
        ...
        mint_buffer[t_index] = min_temp
        maxt_buffer[t_index] = max_temp
        t_index = (t_index+1) % N2
        ave_mint = math.floor(np.mean(mint_buffer))
        ave_maxt = math.ceil(np.mean(maxt_buffer))
        packet_count = (packet_count+1) % CALIBRATION_INTERVAL
        ...
        if (ccount > 0):
            ...
            for c in cntrs:
                ...
                i += 1
        c_buffer[c_index] = ccount
        c_index = (c_index+1) % N1
        ave_count = int(np.round(np.mean(c_buffer)))
        cv2.putText(rgb_frame, str(ave_count), (20,30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 
                    1.0, (0,0,0), 2, cv2.LINE_AA)                 
        cv2.imshow("Thermal Camera", rgb_frame)
        if (packet_count == 0):
            print("Auto calibrating...")   
            if (ave_maxt - ave_mint < NOISE_THRESHOLD):  # avoid noise when the room is empty
                ave_maxt = ave_maxt + NOISE_PEDESTAL
            img = plt.imshow(pixels, 
                    cmap=palette_strings[PALETTE], 
                    interpolation=interpolation_strings[INTERPOLATION],
                    vmin = ave_mint, vmax = ave_maxt)
            print ("ave_min_temp : {} , ave_max_temp : {}".format (ave_mint, ave_maxt))
        ...
        if (key==ord('c')): 
            ...
            if (max_t - min_t < NOISE_THRESHOLD):  # avoid noise when the room is empty
                max_t = max_t + NOISE_PEDESTAL            
            img = plt.imshow(pixels, 
                    cmap=palette_strings[PALETTE], 
                    interpolation=interpolation_strings[INTERPOLATION],
