...
void MLX90621::initialise(int refrate) {
	refreshRate = refrate;
	//Wire.begin(I2C_MASTER, 0, I2C_PINS_18_19, I2C_PULLUP_INT, I2C_RATE_100);   // Rajaraman
  Wire.begin(); // Rajaraman
  // Wire.setClock(100000);  // Rajaraman
	delay(5);
	readEEPROM();
	writeTrimmingValue();
	setConfiguration();
...
void MLX90621::readEEPROM() { // Read in blocks of 32 bytes to accomodate Wire library
  for(int j=0;j<256;j+=32) {
    ...
    Wire.write(j);
    //byte rc = Wire.endTransmission(I2C_NOSTOP);  // Rajaraman
    byte rc = Wire.endTransmission(false);   // false=keep the bus connection alive //     
    if(rc) {
...
void MLX90621::readIR() {
  for(int j=0;j<64;j+=16) { // Read in blocks of 32 bytes to overcome Wire buffer limit   
    ...
    Wire.write(0x01);
    Wire.write(0x20);
    //Wire.endTransmission(I2C_NOSTOP);  // Rajaraman
    Wire.endTransmission(false);   // false=keep the bus connection alive // 
...
uint16_t MLX90621::readConfig() {
	Wire.beginTransmission(0x60);
 // Wire.write() has been replaced by Wire.write() everywhere  // Rajaraman
	Wire.write(0x02);
	Wire.write(0x92);
	Wire.write(0x00);
	Wire.write(0x01);
	//Wire.endTransmission(I2C_NOSTOP);  // Rajaraman
  Wire.endTransmission(false);   // false=keep the bus connection alive // Rajaraman
	Wire.requestFrom(0x60, 2);
